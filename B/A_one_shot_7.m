...
% modified: add other 6 patients Jan/29
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:size(data, 2)
    ...
    % dlarray format ?????????
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    dlx1 = dlarray(single(data(1:12, i)), 'CT');
    dlx2 = dlarray(single(data(13:24, i)), 'CT');
...
for i = 1:size(test_data, 2)
    dlx1 = dlarray(single(test_data(1:12, i)), 'CT');
    dlx2 = dlarray(single(reshape( anchor(1:4,1:3), [12,1])), 'CT');
...
count_y = 1;
for i = 4:n
   negative = length(find(data(i-3:i, 4) == -1));
   positive = length(find(data(i-3:i, 4) == 1));
...
sz = size(p,3);
for i = 1:sz
   data(1:12, i) = reshape(p(1:4,1:3,i), [12,1]); 
   data(13:24, i) = reshape(n(1:4,1:3,i), [12,1]);
   label(i, 1) = 1; 
   
   data(1:12, i+sz) = reshape(n(1:4,1:3,i+2*sz), [12,1]);
   data(13:24, i+sz) = reshape(n(1:4,1:3,i+3*sz), [12,1]);
   label(i+sz,1) = 0;
end
end
